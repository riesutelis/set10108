...
const uint32_t SHA256_par::sha256_k[64] =
...
void SHA256_par::transform(const unsigned char *message, size_t block_nb)
{
    uint32_t w[64];
    uint32_t wv[8];
    uint32_t t1, t2;
...
void SHA256_par::update(const unsigned char *message, size_t len)
{
    size_t block_nb;
...
void SHA256_par::final(unsigned char *digest)
{
    size_t block_nb;
    size_t pm_len;
    size_t len_b;
...
std::string sha256_par(const std::string &input)
{
    ...
    memset(digest, 0, SHA256_par::DIGEST_SIZE);
	SHA256_par ctx = SHA256_par();
    ctx.init();
